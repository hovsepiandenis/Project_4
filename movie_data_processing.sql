...
from (select row_number() over() as rn, bdate from generate_series('1974-01-01'::date, '1985-12-31'::date, '1 day'::interval) as bdate(bdate)) bdate
...
INSERT INTO users (first_name, last_name, nationality, birth_date, is_prime_user, subscription_plan_id)
SELECT DISTINCT
    first_name,
    last_name,
    nationality,
...
UPDATE subscriptions set end_date = start_date + subscription_plan.duration::interval,
...
user_id ,
	...
	select count(user_id) from watched_movies group by movie_id
...
BEGIN
    ...
    LOOP
        ...
        -- Update the watched_movies table
